pipeline {
    ...
    stages {
        ...
        stage('Deploy Container'){
            steps {
                sh '''
                docker ps -q --filter "name=php-webapp" | xargs -r docker stop docker ps -aq --filter "name=php-webapp" | xargs -r docker rm docker run -d --name php-webapp -p 8080:80 php-webapp:latest
                '''
            }
        }
        }
